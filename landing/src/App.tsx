...
const t = {
  ...
  methodTitle: { zh: "调试方法论（三板斜）", en: "Debugging Methodology" },
  ...
  teamTitle: { zh: "Agent Team 集成", en: "Agent Team Integration" },
  teamDesc: { zh: "Leader 统一管理压力等级，Teammate 自驱执行方法论，可选 PUA Enforcer 监工角色。", en: "Leader manages pressure levels globally, Teammates self-drive methodology, optional PUA Enforcer watchdog role." },
}
...
function InstallTabs({ lang }: { lang: Lang }) {
  ...
  const installTabs = [
    ...
    { id: "kiro", label: "Kiro" },
    { id: "project", label: lang === "zh" ? "项目级" : "Project" },
  ]
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              <div className="hero-badge">
                ...
                {lang === "zh" ? "v1.1 · Agent Team 支持" : "v1.1 · Agent Team Support"}
              </div>
            ...
            <GlowBorder>
              <div className="glow-border-inner">
                <p style={{ fontSize: "0.875rem", color: "var(--text-secondary)", lineHeight: 1.75 }}>
                  <strong style={{ color: "var(--accent)" }}>{lang === "zh" ? "关键转折点：" : "Key Turning Point: "}</strong>
                  {lang === "zh"
      ...
      <Sec glow>
        ...
        <Stagger className="responsive-grid grid-3">
          {([
            ...
            { label: lang === "zh" ? "铁律 #3" : "Rule #3", text: L(t.rule3) },
              ...
              {CHECKLIST[lang].map((c, i) => (
                <div key={c.item} className="check-item">
                  ...
                  <div className="check-copy">
                    ...
                    {c.gate && <span className="tag check-gate tag-accent">{lang === "zh" ? "提问门控" : "Ask Gate"}</span>}
                  </div>
                </div>
              ))}
            </div>
