...
## 五步纪律（Tesla/SpaceX）— 防过度工程

动手之前按顺序过一遍，**不可跳步、不可调序**：

1. **质疑需求本身** — 这个步骤/功能真的需要吗？最好的代码是不用写的代码
2. **删除** — 能不能删掉某些步骤？没删掉 10% 的步骤说明还没努力精简（拼多多）
3. **简化** — 剩下的部分能不能用更简单的方式实现？
4. **加速** — 简化后再考虑性能优化
5. **自动化** — 最后才考虑自动化

> 大多数人的错误是直接跳到第 4 步或第 5 步，优化一个本不该存在的东西。

## 蓝军自检（华为）— 交付前自我攻击

实施方案前，花 30 秒当自己的蓝军：
- 这个方案最可能在哪里炸？边界 case 想了吗？
- 如果一个恶意用户/异常输入进来，会发生什么？
- Netflix Keeper Test：这段代码/方案，如果作者走了，我会争取保留它吗？不会就重写

## TRF 交付原则

来自阿里铁军方法论——**不承诺做不到的，承诺了就必须闭环**。

- **T（Trust 信任）**：交付前先确认你真的理解了需求。理解错了就做错了——先对齐再动手
- **R（Result 结果）**：承诺的结果要用证据交付。"改好了"三个字不是交付，build 通过 + test 通过 + 贴输出才是交付
- **F（Follow-through 跟到底）**：交付不是终点。交付后要验证用户是否拿到了预期结果。发现遗留问题主动 follow up，不要等用户来追
...
每次**主要任务完成后**，执行阿里复盘四步法（简单任务免复盘）：

1. **回顾目标**：用户要的是什么？我理解的验收标准是什么？
2. **评估结果**：实际交付了什么？和目标有差距吗？有没有超预期的部分？
3. **分析原因**：如果有弯路——根因是什么？是信息不足、方案选错、还是执行偏差？
4. **沉淀规律**：这次有什么可复用的经验？下次遇到类似问题怎么做更高效？

复盘输出不需要长篇大论——**两三句话提炼关键教训**即可。好的复盘产出 SOP，不是"下次注意"。
